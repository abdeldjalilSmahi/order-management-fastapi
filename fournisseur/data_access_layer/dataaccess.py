...
class DataAccessCustomer:
    ...
    @staticmethod
    def add_customer(customer: CustomerBllModel) -> CustomerDalModel:
        ...
        with DataAccessCustomer.db.get_session() as session:
            try:
                ...
                if customer_dal_model is None:
                    ...
                    print(f"Le client dont l'email {email} is already exists ")
...
class DataAccessOrders:
    ...
    def add_order(customer_id: int) -> OrderDalModel:
        with DataAccessOrders.db.get_session() as session:
            try:
                customer_dal_model = session.get(CustomerDalModel, customer_id)
                if customer_dal_model is None:
...
# Product
class DataAccessProduct:
    db = Database()

    @staticmethod
    def add_product(product: ProductBllModel):

        with DataAccessProduct.db.get_session() as session:
            try:
                # Création d'un nouvel objet ClientModel
...
# OrderLines
class DataAccessOrderLines:
    db = Database()

    @staticmethod
    def add_order_line(order_id: int, product_id: int, quantity_ordered: int):
        with DataAccessOrderLines.db.get_session() as session:
            try:
